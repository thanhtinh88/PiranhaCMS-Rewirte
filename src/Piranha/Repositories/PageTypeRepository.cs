...
namespace Piranha.Repositories
{
    public class PageTypeRepository: IPageTypeRepository
    {
        ...
        public Models.PageType GetById(string id, IDbTransaction transaction = null)
        {
            ...
            if (model == null)
            {
                var type = db.QueryFirstOrDefault<PageType>($"SELECT * FROM [{table}] WHERE [Id]=@Id",
        ...
        {
            if ((int)db.ExecuteScalar($"SELECT COUNT([Id]) FROM [{table}] WHERE [Id]=@Id",
        ...
        /// <param name="model">The model</param>
        /// <param name="transaction">The optional transaction</param>
        private void Add(Models.PageType model, IDbTransaction transaction)
        {
        ...
        /// <param name="model">The model</param>
        /// <param name="transaction">The optional transaction</param>
        private void Update(Models.PageType model, IDbTransaction transaction)
        {
            var pageType = db.QueryFirst<PageType>($"SELECT * FROM [{table}] WHERE [Id]=@Id",
                new { Id = model.Id });
